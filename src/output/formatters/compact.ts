...
export function formatAsCompact(findings: Finding[], filesScanned: number, duration: number): string {
  if (findings.length === 0) {
    return chalk.green(`${logSymbols.success} No issues found (${filesScanned} files, ${duration}ms)`);
  }
