...
export abstract class BaseRule implements Rule {
  ...
  protected execWithTimeout(
    ...
    if (content.length > maxContentLength) {
      console.warn(`File too large (${content.length} bytes), truncating to ${maxContentLength} bytes`);
    ...
    try {
      ...
      console.warn(`Regex execution error: ${error instanceof Error ? error.message : String(error)}`);
    }
